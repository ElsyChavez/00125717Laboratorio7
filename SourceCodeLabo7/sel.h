...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau, kappa, lambda, f_x, f_y, delta, eta,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD, matrixP, matrixH;
    ...
    delta = m.getParameter(CONST_DELTA);
    ...
    //matriz P
    productRealMatrix((-1)*(delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixP);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixH,matrixP,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(CONST_X);
    float f_y = m.getParameter(CONST_Y);
    float eta = m.getParameter(CONST_ETA);
